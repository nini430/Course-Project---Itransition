...
const generateItemValidationSchema=(formikFields:any)=>{
        let validationSchema=object().shape({
            ...
            tags:array().of(string().required('field_required')).required().test({name:'not Empty',test:value=>value.length>0,message:'must_not_be_empty'})
        }) as any;

        if(formikFields['integer']) {
            formikFields['integer'].map((item:any)=>{
                validationSchema=validationSchema.shape({
                    [item.name]:number().required(`field_required`)
                })
            })
        }
        if(formikFields['string']) {
            formikFields['string'].map((item:any)=>{
                validationSchema=validationSchema.shape({
                    [item.name]:string().required(`field_required`)
                })
            })
        }
        ...
        if(formikFields['boolean']) {
            formikFields['boolean'].map((item:any)=>{
                validationSchema=validationSchema.shape({
                    [item.name]:boolean().required(`field_required`)
                })
            })
        }
