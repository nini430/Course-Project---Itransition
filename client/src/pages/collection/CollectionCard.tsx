...
const CollectionCard = ({ currentCollection }: ICollectionCardProps) => {
  ...
  const { uploadCollectionImageLoading, removeCollectionLoading } =
      ...
      <AccordionDetails sx={{ display: 'flex', flexDirection: 'column' }}>
        <CardContainer>
          ...
          >
            ...
            <ImageUpload
              getRootProps={
                (currentCollection?.author.id === authedUser?.id || authedUser?.role==='ADMIN' ) ? getRootProps : null
              }
              getInputProps={
                (currentCollection?.author.id == authedUser?.id || authedUser?.role==='ADMIN') ? getInputProps : null
              }
              isAllowedUpload={(currentCollection?.author.id === authedUser?.id)||authedUser?.role==='ADMIN'}
              uploadImage={async () => {
                dispatch(
                  uploadCollectionImage({
            ...
            >
              ...
              currentCollection?.description.length > 50 ? (
                <ShowMore
                  children={
                    (
                      <Typography
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <Avatar
                    ...
                    height={30}
                    src={
                      (currentCollection?.author.profileImage as string) ||
                      AvatarImage
                    }
                  />
                  <Typography>
        ...
        {(authedUser?.id === currentCollection?.author.id) || authedUser?.role==='ADMIN' && (
          <ActionsContainer>
            <Link
              to={`/edit-collection/${currentCollection?.id}`}
              style={{ textDecoration: 'none' }}
            >
              <IconButton>
                <Edit />
